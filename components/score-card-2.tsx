...
function CandidateCard({ candidate, selected }: { candidate: Candidate; selected: boolean }) {
  ...
  const startAnimation = useCallback(async () => {
    if (selected) {
      ...
      await animate(".card", { scale: 0.95 }, { duration: 0.3, ease: "easeInOut" });
      animate(".score-text", { opacity: 1, y: 0 }, { duration: 0.3, ease: "easeInOut" });
    } else {
      animate(
        ".card",
      ...
      await animate(".card", { scale: 1 }, { duration: 0.3, ease: "easeInOut" });
      animate(".score-text", { opacity: 0, y: -30 }, { duration: 0.3, ease: "easeInOut" });
    }
  }, [animate, selected]);
  ...
  return (
    <motion.div ref={scope}>
      <div className="relative card">
        ...
        <motion.span
          className="score-text absolute inset-x-0 bottom-4 m-auto h-fit w-fit rounded-md border border-neutral-100 bg-white px-2 py-1 text-xs text-black"
